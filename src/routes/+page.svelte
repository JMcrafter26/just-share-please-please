<script>
	...
	// ---- Copy button for code blocks (GitHub-style) ----
	...
	onMount(() => {
		...
		// minimize Import button when not at top
		let ticking = false;
		const onScroll = () => {
			if (ticking) return;
			ticking = true;
	...
	// Keep code-block buttons in sync if bodyHtml changes outside display() flow
	$effect(() => {
		void bodyHtml;
...
<a href={obsidianHref} class="obsidian-top-button" class:obsidian-compact={isObsidianCompact} title="Import this note into Obsidian" aria-label="Import this note into Obsidian">
	...
	<span class="obsidian-label">Import into Obsidian</span>
</a>
